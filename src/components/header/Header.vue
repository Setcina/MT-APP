<template>
  <div class="header">
    ...
    <div class="bulletin-wrapper">
      <img class="icon" v-if="Info.discounts2" :src="Info.discounts2[0].icon_url" />

      <span class="text" v-if="Info.discounts2">{{Info.discounts2[0].info}}</span>
      <div class="detail" v-if="Info.discounts2" @click="showBulletin">
        {{Info.discounts2.length}}个活动
      ...
      <div class="bulletin-detail" v-show="isShow">
        <div class="detail-wrapper">
          ...
          <div class="main-wrapper" :style="detail_bg">
            ...
            <!-- 星级评价 -->
            <div class="score">
              <app-star :score="Info.wm_poi_score"></app-star>
              <span>{{Info.wm_poi_score}}</span>
            </div>

            <p class="tip">
              {{Info.min_price_tip}} <i>|</i> {{Info.shipping_fee_tip}} <i>|</i> {{Info.delivery_time_tip}}
              </p>
            ...
            <div class="discounts" v-if="Info.discounts2">
              <p>
                <img :src="Info.discounts2[0].icon_url" />
                <span>{{Info.discounts2[0].info}}</span>
              </p>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      head_pic_url(){
        return "background-image: url(" + this.Info.head_pic_url + ");"
      },
      head_bg(){
        return "background-image: url(" + this.Info.pic_url + ");"
      },
      detail_bg(){
        return "background-image: url(" + this.Info.poi_back_pic_url + ");"
      }
    },
    methods:{
      showBulletin(){
