      ...
      <div class="food-wrapper">
        <div class="food-content">
          <div class="img-wrapper">
            <img class="food-img" :src="seleteFood.picture"/>
            <span class="close-bt icon-close" @click="closeFlags"></span>
            <img class="share-bt" src="./img/share.png" />
            <img class="more-bt" src="./img/more.png" />
          </div>
          <div class="content-wrapper">
            <h3 class="name">{{seleteFood.name}}</h3>
            <p class="saled">{{seleteFood.month_saled_content}}</p>
            <img class="product" v-show="seleteFood.product_label_picture" :src="seleteFood.product_label_picture"/>
            <p class="price">
              <span class="text">￥{{seleteFood.min_price}}</span>
              <span class="unit">/{{seleteFood.unit}}</span>
            </p>

            <div class="cartcontrol-wrapper">
              <app-cartControl :food="seleteFood"></app-cartControl>
            </div>
            <div class="buy" v-show="!seleteFood.count||seleteFood==0" @click="addProduct">
              选规格
            </div>
          </div>
        </div>
        ...
        <div class="rating-wrapper">
          ...
          <div class="rating-title">
            <div class="like-ratio" v-if="seleteFood.rating">
              <span class="title">{{seleteFood.rating.title}}</span>
              <span class="retio">
                (
                  {{seleteFood.rating.like_ratio_desc}}
                  <i>{{seleteFood.rating.like_ratio}}</i>
                )
              </span>
            </div>
            <div class="snd-title" v-if="seleteFood.rating">
                  <span class="text">{{seleteFood.rating.snd_title}}</span>
                  <span class="icon icon-keyboard_arrow_right"></span>
            </div>
          </div>
          <ul class="rating-content" v-if="seleteFood.rating">
            <li
              v-for="(comment,index) in seleteFood.rating.comment_list"
              :key="index"
              class="comment-item"
              >
...
export default {
  ...
  methods:{
    ...
    addProduct(){
      vue.set(this.seleteFood,"count",1)
    }
  }
}
</script>
